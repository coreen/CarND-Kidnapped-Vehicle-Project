...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
  ...
  for (int i = 0; i < particles.size(); i++) {
    ...
    if (yaw_rate == 0) {
      ...
      new_x = p.x + coefficient * (sin(parameter) - sin(std_pos_theta));
      new_y = p.y + coefficient * (cos(std_pos_theta) - cos(parameter));
      new_theta = p.theta + yaw_rate * delta_t;
    }
  
    // add noise
    normal_distribution<double> dist_x(new_x, std_pos_x);
    normal_distribution<double> dist_y(new_y, std_pos_y);
    normal_distribution<double> dist_theta(new_theta, std_pos_theta);
